...
func TestCubic(t *testing.T) {
	...
	sVals := getTestGreyImage(testImage50)
	for y, ys := range sVals {
		for x, v := range ys {
			s := Sample2D{Pos2D{float64(x) * imgFac, float64(y) * imgFac}, v}
	...
	go func() {
		for x := 0; x < imgSize; x++ {
			for y := 0; y < imgSize; y++ {
				positions <- Pos2D{float64(x) + 0.5, float64(y) + 0.5}
			}
		}
		close(positions)
	}()
	...
	for sample := range interp.Multi(positions) {
		...
		c := uint16(sample.Val * float64(math.MaxUint16))
